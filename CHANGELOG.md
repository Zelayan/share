...
- 热搜结果页“和当前人数一起讨论”底栏复用正文评论页的悬浮胶囊：48dp 高、24dp 圆角、88% 主题表面、左右 16dp 边距，并保留原讨论 / 分享 / 快捷操作。
- 热搜结果页内容改为 edge-to-edge，仅保留顶部系统 inset；胶囊使用 24dp 手势区兜底加 8dp 间距，避免覆盖系统手势横线，底部列表占位同步为 56dp。
- 新增 `scripts/test_search_result_action_bar.sh`，覆盖布局组件、边距、高度、滚动行为、列表占位和 SearchActivity inset 生命周期。
- 验证：全部 `scripts/test_*.sh` 静态回归通过；临时签名 APK 已安装到 Xiaomi `24129PN74C` Android 16 实测样式和反向滚动恢复。正式 Build Tools 对齐 / v2-v3 签名产物待后续重建。
